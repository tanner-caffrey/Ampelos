...
export class ModuleLoader {
  ...
  private async loadModule(modulePath: string, expectedType: 'tool' | 'service' | 'both'): Promise<void> {
    try {
      ...
      if (expectedType === 'tool' && !manifest.provides.includes('tool')) {
        log.warn('Module in tools/ but does not provide tool', { module: manifest.name });
        return;
      }
      if (expectedType === 'service' && !manifest.provides.includes('service')) {
        log.warn('Module in services/ but does not provide service', { module: manifest.name });
        return;
      }
      ...
      if (manifest.provides.includes('tool')) {
        try {
          ...
          // Also load Letta tools if exported (Python tools that run on Letta server)
          if (toolModule.lettaTools && Array.isArray(toolModule.lettaTools)) {
            loadedModule.lettaTools = toolModule.lettaTools as LettaToolDefinition[];
          }
        } catch (error) {
      ...
      // Store module by manifest.name (not directory name)
      // This allows agent configs to reference modules by their manifest name
  ...
  /**
   * Get a loaded module by manifest.name (not directory name)
   * Modules are keyed by their manifest.name throughout the system
   */
