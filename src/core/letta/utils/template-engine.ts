...
// Base path for templates (relative to core/letta/utils -> core/letta/templates)
const TEMPLATES_DIR = resolve(__dirname, '..', 'templates');

// Allowed directories for file references (security: prevent arbitrary file reads)
...
function validateFilePath(filePath: string): string {
  ...
  const isAllowed = ALLOWED_FILE_DIRS.some(dir =>
    resolved.startsWith(dir + '/') || resolved === dir
  );
  if (!isAllowed) {
    throw new Error(
      `File path not allowed: ${filePath}. Must be within configs/ or templates/`
    );
  }
  return resolved;
}
...
export async function processMemoryBlock(
  ...
  // Mode 3: File reference (validated for security)
  if (config.file) {
    try {
      const validatedPath = validateFilePath(config.file);
...
export async function processSystemPrompt(
  ...
  // Check if it's a file path (validated for security)
  try {
    const validatedPath = validateFilePath(config);
    const content = await readFile(validatedPath, 'utf-8');
    return renderTemplate(content, variables);
  } catch {
    // If file doesn't exist or path not allowed, treat as direct string
    return renderTemplate(config, variables);
  }
}
